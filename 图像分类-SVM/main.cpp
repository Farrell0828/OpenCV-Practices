...
private:
	...
	Ptr<SurfFeatureDetector> featureDecter;
	Ptr<SurfDescriptorExtractor> descriptorExtractor;
...
categorizer::categorizer(int _clusters)
{
	...
	clusters = _clusters;
	//初始化指针
	featureDecter = SurfFeatureDetector::create();
...
void categorizer::trainSvm()
{
...

	{
		...
		for (int i = 0; i < categories_size; i++)
		{
			...
			stor_svms[i]->setTermCriteria(cvTermCriteria(CV_TERMCRIT_ITER, 100, 1e-6));
		}
		...
		for (int i = 0; i<categories_size; i++)
		{
			...
			//设置训练数据 
			Ptr<TrainData> tData = TrainData::create(tem_Samples, ROW_SAMPLE, responses);
			stor_svms[i]->train(tData);
...
void categorizer::category_By_svm()
{
	...
	for (; begin_train != end_train; ++begin_train)
	{
		...
		for (int i = 0; i<categories_size; i++)
		{
			...
			//读取SVM.xml
			Mat output_array;	// 试验参数
			if (svm_fs.isOpened())
			{
				svm_fs.release();
				Ptr<SVM> st_svm = SVM::load(String(f_path));
				
				if (sign == 0)
				{
					float score_Value = st_svm->predict(test, output_array, cv::ml::StatModel::RAW_OUTPUT);
					float class_Value = st_svm->predict(test);
					sign = (score_Value < 0.0f) == (class_Value < 0.0f) ? 1 : -1;
				}
				curConfidence = sign * st_svm->predict(test, output_array, cv::ml::StatModel::RAW_OUTPUT);
			}
			else
			{
				if (sign == 0)
				{
					float scoreValue = stor_svms[i]->predict(test, output_array, cv::ml::StatModel::RAW_OUTPUT);
					float classValue = stor_svms[i]->predict(test, Mat(), false);
					sign = (scoreValue < 0.0f) == (classValue < 0.0f) ? 1 : -1;
				}
				curConfidence = sign * stor_svms[i]->predict(test, output_array, cv::ml::StatModel::RAW_OUTPUT);
			}
