...
int main()
{
	...
	Ptr<SurfFeatureDetector> detector =
		SurfFeatureDetector::create(800);//创建SURF特征检测器
	// Ptr<SURF> detector = SURF::create(800);

	Ptr<SurfDescriptorExtractor> descriptor_extractor =
		SurfDescriptorExtractor::create(800);//创建特征向量生成器
	...
	//cvtColor(img1_raw, img1, CV_BGR2GRAY);
	//cvtColor(img2_raw, img2, CV_BGR2GRAY);
